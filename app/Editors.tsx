...
function Trigger({
  ...
  empty?: boolean;
  busy: boolean;
  /** The control's job, for its accessible name. The visible content is the
   *  current VALUE, which reads as a fact — the state-pill lesson, applied
   *  to the whole column. */
  what: string;
}) {
  return (
    ...
    <dl className="grid grid-cols-[7rem_1fr] items-center gap-y-0.5 text-[13px]">
      <Row label="Assignee">
        <DropdownMenu onOpenChange={(open) => { if (open) options.want(); }}>
          <Trigger what="assignee" busy={busy} empty={fields.assignee === null}>
      ...
      <Row label="Priority">
        <DropdownMenu onOpenChange={(open) => { if (open) options.want(); }}>
          <Trigger what="priority" busy={busy} empty={fields.priority === 0}>
      ...
      {detail.usesEstimates ? (
        <Row label="Estimate">
          <DropdownMenu onOpenChange={(open) => { if (open) options.want(); }}>
            <Trigger what="estimate" busy={busy} empty={fields.estimate === null}>
      ...
      <Row label="Labels">
        <DropdownMenu onOpenChange={(open) => { if (open) options.want(); }}>
          <Trigger what="labels" busy={busy} empty={detail.labels.length === 0}>
      ...
      <Row label="Cycle">
        <DropdownMenu onOpenChange={(open) => { if (open) options.want(); }}>
          <Trigger what="cycle" busy={busy} empty={fields.cycleId === null}>
...
function PriorityMark({ priority }: { priority: number }) {
  ...
  // No priority: a dashed outline circle, not empty bars — "none" is a
  // different statement from "the lowest one". An Icon rather than a drawn
  // mark, with the same classes as the sibling rows' person/tag/folder
  // glyphs, because the empty states form a column of their own and this one
  // must not be the row in a different ink.
  return <Icon name="CircleDashed" className="size-3.5 shrink-0 opacity-60" aria-hidden />;
}
